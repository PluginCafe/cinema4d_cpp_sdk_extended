...
maxon::Result<void>InstantiateDescID(cinema::BaseDocument* doc);

/// @brief Demonstrates how to access the data container of scene elements.
maxon::Result<void>AccessNodeDataContainer(cinema::BaseDocument* doc);

/// @brief Demonstrates the slightly changed syntax of accessing node branches.
maxon::Result<void>AccessNodeBranches(cinema::BaseDocument* doc);

/// @brief Demonstrates the copy-on-write changes made to `VariableTag` and `BaseSelect`.
maxon::Result<void>CopyOnWriteSceneData(cinema::BaseDocument* doc);
