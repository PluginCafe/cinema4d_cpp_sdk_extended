...
public:
		...
		const maxon::GetExtraDataCallbackType& getExtraDataDelegate, BaseDocument* doc) const
	{
		...
		iCustomDataTypeDots defaultData = dataEntry.Get(maxon::DESCRIPTION::DATA::BASE::DEFAULTVALUE.Get(),
														 iCustomDataTypeDots());
		GeData value = GeData(id_sdk_example_customdatatype_dots, 
								reinterpret_cast<const CustomDataType&>(defaultData));
		...
		const maxon::GetExtraDataCallbackType& extraDataDelegate, BaseDocument* doc) const
	{
		...
		output = GeData(id_sdk_example_customdatatype_dots,
						 reinterpret_cast<const CustomDataType&>(value));
		...
		const maxon::GetExtraDataCallbackType& extraDataDelegate, BaseDocument* doc) const
	{
		...
		iCustomDataTypeDots dotData = *(static_cast<iCustomDataTypeDots*>(data.GetCustomDataType(
															id_sdk_example_customdatatype_dots)));
