...
maxon::Result<void> LatticePlaneModifier::PrepareFFD(maxon::BaseArray<FFD_Data>& pointsFFD, PointObject* opPointObj, PointObject* modPointObj)
{
	iferr_scope;
...
maxon::Result<void> LatticePlaneModifier::EvaluateFFD(PointObject* opPointObj, PointObject* modPointObj, const maxon::BaseArray<FFD_Data>& pointsFFD)
{
	iferr_scope;
...
maxon::Result<void> LatticePlaneModifier::DrawPoints(BaseDraw* bd, BaseSelect* pointsBS, const Vector* pointsR, const Int32 pointsCnt)
{
	iferr_scope;
...
Bool LatticePlaneModifier::GetDEnabling(GeListNode* node, const DescID& id, const GeData& t_data, DESCFLAGS_ENABLE flags, const BaseContainer* itemdesc)
{
	...
	switch (id[0].id)
	{
		case SDK_EXAMPLE_LATTICEPLANEMODIFIER_NORMALTHR:
			...
			node->GetParameter(SDK_EXAMPLE_LATTICEPLANEMODIFIER_ENABLENORMALCHK, enableNormalCheckData, DESCFLAGS_GET::NONE);
...
Bool LatticePlaneModifier::ModifyObject(BaseObject* mod, BaseDocument* doc, BaseObject* op, const Matrix& op_mg, const Matrix& mod_mg, Float lod, Int32 flags, BaseThread* thread)
{
	...
	iferr (EvaluateAndStoreVertexNormals(pointsFFD, opPointObj))
	{
	...
	iferr (PrepareFFD(pointsFFD, opPointObj, modPointObj))
	{
		DiagnosticOutput("Error on PrepareFFD: @", err);
		return false;
	}
	// Use the stored FFD_Data to evaluate deformation.
	iferr (EvaluateFFD(opPointObj, modPointObj, pointsFFD))
	{
