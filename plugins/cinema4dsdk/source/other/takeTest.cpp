    ...
    case ID_CREATETAKE:
    {
      ...
      // just if we are in an user defined take
        ...
        if (overrideNOde && overrideNOde->IsOverriddenParam(DescLevel(PRIM_CUBE_SUBY, DA_LONG, 0))) // if we find the overridden param
				{
          overrideNOde->SetParameter(DescLevel(PRIM_CUBE_SUBY, DA_LONG, 0), GeData(20), DESCFLAGS_SET::NONE); // we can just change the value like in all other nodes with SetParameter
          ...
          BaseOverride* counterpartNode = takeData->FindOverrideCounterPart(overrideNOde, DescLevel(PRIM_CUBE_SUBY, DA_LONG, 0), resultTake); // now we search the node whenre the backup value is stored
            ...
            counterpartNode->SetParameter(DescLevel(PRIM_CUBE_SUBY, DA_LONG, 0), GeData(0), DESCFLAGS_SET::NONE); // change the value also here for example
		...
		case ID_AUTOTAKE:
		{
			...
			if (!cTake->IsMain() && op->IsInstanceOf(Ocube))
			{
				...
				op->SetParameter(DescLevel(PRIM_CUBE_SUBY, DA_LONG, 0), GeData(20), DESCFLAGS_SET::NONE);
